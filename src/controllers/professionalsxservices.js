...
module.exports = {
    ...
    async index(request, response) {
        ...
        try {
            ...
            const services = await connection('professionalxservices')
                .where({ id: id })
                .select('*')
    ...
    async create(request, response) {
        try {
            const { idServices, idProfessional } = request.body

            await idServices.forEach(async idServices => {

                const services = await connection('professionalxservices').insert({
                    idProfessional,
                    idServices
                }).then(res => {
                    ...
                    if(err.code && err.code == '23503'){
                        ...
                        message = 'Erro ao tentar inserir serviço x profissional'
                    }
